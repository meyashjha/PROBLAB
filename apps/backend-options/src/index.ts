...
// Routes
import probabilityOptionRoutes from './routes/probabilityOption.routes';
...
app.use('/api/events', readRateLimiter, jupiterRoutes);

// Options routes - apply read rate limiter (individual routes handle write protection)
app.use('/api/options', readRateLimiter, probabilityOptionRoutes);
...
async function start() {
  try {
    ...
    // Start the market snapshot scheduler (saves probability snapshots every 15 min)
    marketSnapshotService.start();
    
    app.listen(env.PORT, () => {
      console.log(`🚀 Options Service running on port ${env.PORT}`);
    });
  } catch (error) {
